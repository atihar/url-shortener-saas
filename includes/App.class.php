...
class App{
			...
			if($user = $this->db->get("user",array("id"=>"?","auth_key"=>"?"),array("limit"=>1),array($info[0],$info[1]))){
			    ...
			    if(!isset($plan->permission->parameters)) {
			     ...
			     if(!isset($plan->permission->multiple)) {
			      $this->user->plan->permission->multiple = new stdClass;
			      $this->user->plan->permission->multiple->enabled = 0;
			    }
				}
		...
		return Main::redirect("?quick.error");					
		 ...
		 /**
	      ...
	      $query .= ") AND userid=:user";				
				...
				echo "<section id='last_history' class='dark urls-holder styled'>";
	...
	private function checkout(){
		...
		if(isset($_GET["trial"]) && $plan->trial_days){
			...
			if($this->db->get("payment", "trial_days IS NOT NULL AND userid = '{$this->user->id}'", ["limit" => 1])){
				return Main::redirect(Main::href("pricing","",FALSE), ["danger", e("You have already used a trial.")]);
			}
			...
			$this->db->update("user", [
				...
				"expiration" => date("Y-m-d H:i:s", strtotime("+ {$plan->trial_days} days"))
			], ["id" => $this->user->id]);
    		...
    		":userid"  =>  $this->user->id,
    		":status" => "Completed",
    		":expiry" =>  date("Y-m-d H:i:s", strtotime("+ {$plan->trial_days} days")),
    		":data" =>  NULL
    		];
	...
	protected function webhook(){
		...
		$fn = "webhook_{$this->do}";
		if(in_array($this->do, ["slack","paypal"]) && method_exists(__CLASS__, $fn)){
			return $this->$fn();
		}				
	 ...
	 * @return  [type] [description]
	 */
	protected function webhook_slack(){
		
		$start = microtime(true);

		include(ROOT."/includes/library/Slack.class.php");
		if(\kbrmedia\Slack::validate($this->config["slacksigningsecret"])){

			$user_id = $_POST["user_id"];
			$content = $_POST["text"];
			$webhook = $_POST["response_url"];
			...
			$short = new Short($this->db,$this->config);
			if($custom) $array["custom"] = Main::slug($custom);
			$array["url"] = Main::clean($url, 3, TRUE);

			$result = $short->add($array, ["noreturn" => TRUE, "api" => TRUE, "user" => $user]);
			...
			if((microtime(true) - $start) > 3){
				...
				Main::curl($webhook, [
						"post" => true, 
						"json" => true, 
						"body" => json_encode(["text" => isset($result["short"]) ? $result["short"] : $url])
					]);
				return true;
			}
			
			return print(isset($result["short"]) ? $result["short"] : $url);
		}
		return print("Error");
	}
	/**
	...
	private function ipn(){
		// If demo mode is on disable this feature
		if($this->config["demo"]){
			return Main::redirect(Main::href("user","",FALSE),array("danger",e("Feature disabled in demo.")));
		}	
		// Disable Pro membership
		if(!$this->config["pro"]) return $this->_404();
	...
	protected function profile(){
		...
		$profile = json_decode($user->profiledata, TRUE);
		...
		if(!empty($this->id) && is_numeric($id[0])){
			...
			$page="u/{$user->username}/".Main::slug($bundle->name)."-".Main::slug($bundle->id);			
		}else{
			// Get URLs
			$urls=$this->db->get("url",array("userid"=>$user->id,"public"=>1),array("order"=>"date","limit"=>(($this->page-1)*$this->limit).", {$this->limit}","count"=>TRUE));			
			if($profile["name"]){
				Main::set("title","".$profile["name"]." ".e("Profile"));
			}else{
				Main::set("title","@".ucfirst($user->username)." ".e("Profile"));
			}
    ...
    if($this->page > 1 && $this->page > $max) return Main::redirect("u/{$user->username}",array("danger","No URLs found."));
		...
		if($this->sandbox == TRUE) {
			...
			Main::add("<link rel='stylesheet' type='text/css' href='{$this->config["url"]}/themes/{$this->config["theme"]}/style.css'>","custom",false);
	 ...
	 * @return  [type]       [description]
	 */
	protected function profile_qr($user){
		...
		$api_url = "http://chart.apis.google.com/chart?chs=$size&chld=L|0&choe=UTF-8&cht=qr&chl=".Main::href("u/{$user->username}?source=qr");	
	...
	*/
	...
	protected function contact(){
		...
		if(!$this->config["contact"]) return $this->_404();
	...
	protected function analytic(){
	  ...
	  if($this->config["tracking"] == "1" || $this->config["tracking"] == "2"){
			...
			if(isset($_GET["from"]) && isset($_GET["to"])){
				$from = Main::clean($_GET["from"],3,TRUE);
				$to = Main::clean($_GET["to"],3,TRUE);
			}

	  	$clicks	=	$this->stats_chart($alias,$total, $from, $to);
	  	$countries	=	$this->stats_countries($alias, $from, $to);	  	
	  	$country	=	$countries[0];
	  	$top_country	=	$countries[1];
	  	$data	=	$this->stats_referrers($alias, $from, $to);
	  	$referrers	=	$data[0];
	  	$fb	=	$data[1];
	  	$tw	=	$data[2];
	  	$gl	=	$data[3];
		...
		include(ROOT."/includes/analytics.php");
			 ...
			 * @since 5.7
			 **/
			private function stats_chart($id, $click, $from = null, $to = null){
		    ...
		    $clicks = array();
		      ...
		      $clicks[$dt->format("Ymd")]=0;		
				}  
        $data=Main::cache_get("url_click_daily_$id");		        
        if($data == null){
          $data=$this->db->get(array("count"=>"COUNT(DATE(date)) as count, DATE(date) as date","table"=>"stats"),"urlid='$id' AND (date BETWEEN '{$from}' AND '{$to}')",array("group_custom"=>"DATE(date)"));  
          if($click > 1000){
			 ...
			 * @since 5.7
			 **/
			private function stats_countries($id, $from = null, $to = null){
    		$country=array();
    		$top_country=array();				
				$this->db->object=TRUE;
        ...
        if($data == null){
          $data=$this->db->get(array("count"=>"country AS country, COUNT(country) AS count","table"=>"stats"),"urlid='$id' AND (date BETWEEN '{$from}' AND '{$to}')",array("group"=>"country","order"=>"count"),array($id));  
			 ...
			 * @since 4.0
			 **/
			private function stats_referrers($id, $from = null, $to = null){
				$domains = array();
				if(!$from || !$to){
					$from = date("Y-m-d", strtotime("-14 days"));
					$to = date("Y-m-d", strtotime("+1 day"));
		    ...
		    $data=$this->db->get(array("count"=>"domain AS domain, COUNT(domain) AS count","table"=>"stats"),"urlid='$id' AND (date BETWEEN '{$from}' AND '{$to}')", array('group' => "domain","limit"=>10),array($id));
		    $browser = $this->db->get(array("count"=>"browser as browser, COUNT(browser) AS count","table"=>"stats"),"urlid='$id' AND (date BETWEEN '{$from}' AND '{$to}')", array('group' => "browser","limit"=>10, "order" => "count"),array($id));
		    $os = $this->db->get(array("count"=>"os as os, COUNT(os) AS count","table"=>"stats"),"urlid='$id' AND (date BETWEEN '{$from}' AND '{$to}')", array('group' => "os","limit"=>10,"order" => "count"),array($id));		    
		    $fb = $this->db->count("stats","urlid='$id' AND (domain LIKE '%facebook.%' OR domain LIKE '%fb.%') AND (date BETWEEN '{$from}' AND '{$to}')");
		    $tw = $this->db->count("stats","urlid='$id' AND (domain LIKE '%twitter.%' OR domain LIKE '%t.co%') AND (date BETWEEN '{$from}' AND '{$to}')");
		    $gl = $this->db->count("stats","urlid='$id' AND (domain LIKE '%plus.url.google%') AND (date BETWEEN '{$from}' AND '{$to}')");
	...
	protected function unsafe(){
		...
		$this->header();
		if(file_exists(TEMPLATE."/shared/unsafe.php")) {
			include($this->t("shared/unsafe"));			
		} else {		
			$content="<section class='red rounded wshadow'><h1><i class='glyphicon glyphicon-info-sign'></i></h1>
							<h2>Unsafe Link Detected</h2> <p style='color:#fff'>We have detected that the link you were about to be redirected is not safe so we blocked it. If you have any questions, feel free to contact us.</p></section>";							
	...
	protected function _DNS404($domain){
		...
		if(file_exists(TEMPLATE."/shared/customdomain.php")) {
			include($this->t("shared/customdomain"));			
		} else {
	...
	protected function _404(){
		...
		$domain = parse_url($current);

		if($current != $this->config["url"] && $url = $this->db->get("domains", ["domain" => "?"], ["limit" => 1], [$domain["scheme"]."://".$domain["host"]])){
			header("Location: {$url->redirect404}");
			exit;
		}
	...
	protected function header(){
		...
		if(!empty($this->config["analytic"])){					
			Main::add("<script async src='https://www.googletagmanager.com/gtag/js?id={$this->config["analytic"]}'></script><script>window.dataLayer = window.dataLayer || [];function gtag(){dataLayer.push(arguments);}gtag('js', new Date());gtag('config', '{$this->config["analytic"]}');</script>","custom",FALSE);
		}			
	...
	protected function footer(){
		if(!empty($this->config["customfooter"])){
			Main::add(html_entity_decode($this->config["customfooter"]),"custom",TRUE);
		}
		...
		return;
			...
			protected function shortener_option($form=FALSE){
				...
				if($this->config["multiple_domains"]){
					...
					if(!$this->config["root_domain"] || ($this->logged() && $this->permission("multiple") !== FALSE)){
						$domains = explode("\n", $this->config["domain_names"]);
						foreach ($domains as $domain) {
							if(!empty($domain)) $html.='<option value="'.strtolower(trim($domain)).'"'.(($this->logged() && $this->user->domain==$domain)?' selected':'').'>'.str_replace("https://","",str_replace("http://", "", trim($domain))).'</option>';
						}
					}				
		...
		$menu='<div class="navbar-collapse collapse">';
          ...
          $menu.="<li class='dropdown'>
					          ...
					          <ul>
			          			...
			          			<li><a href='".Main::href("user/builder")."'><span class='glyphicon glyphicon-leaf'></span> ".e('Profile Builder')."</a></li>
		...
		private function server_die(){
			...
			private function server_bulk_bundle(){
				if($bundles = $this->db->get("bundle",array("userid"=>"?"),"", array($this->user->id))){
					...
					echo e('No bundles found..').'<br><br> <a href="#" data-action="bundle_create" data-title="'.e("Create Bundle").'" class="btn btn-sm btn-primary ajax_call">'.e("Create Bundle").'</a>';
				}			
			}	
			/**
			 ...
			 * @return  [type] [description]
			 */
			private function server_validatecoupon(){

				if($coupon = $this->db->get("coupons", ["code" => "?"], ["limit" => 1], [Main::clean($_POST["code"], 3, TRUE)])){
					if(strtotime("now") > strtotime($coupon->validuntil)) return print("invalid");
					return print($coupon->description);
				}
	...
	protected function widgets($widget, $option=array()){
		...
		protected function widget_activities($option=array()){
      	...
      	if(empty($data)){
        	...
        	$html.="<ul>";
        		foreach ($data as $item) {
        			$url = $this->db->get(array("count" => "meta_title, domain", "table" => "url"),"BINARY alias=:q OR BINARY custom=:q",array("limit"=>1),array(":q"=>$item->short));
		...
		protected function widget_countries($option=array()){
			...
			if(isset($option["urlid"])) {
				$where=array("urlid"=>Main::clean($option["urlid"],3,TRUE));
			}else{
				$option["urlid"]="";
   ...
   * @return  [type] [description]
   */
  protected function bookmarkletjs(){

  	header("Content-type: text/javascript");
    ob_start("compress");
  	$js = file_get_contents(ROOT."/static/bookmarklet-file.js");
	...
	protected function api(){				
		...
		return new API($this->db,$this->config,$this->do, $this->id);			
	}
	/**
	 * Blog
	 * @author KBRmedia <https://gempixel.com>
    ...
    $post->content = str_replace("{{--more--}}", "", $post->content);
		...
		if($post->image) Main::set("image", uploads("blog/{$post->image}"));
  ...
  public static function device() { 
	...
	public function country($ip=NULL, $api=''){
		...
		require_once ROOT."/includes/library/geoip/autoload.php";
			...
			$reader->close();
			if(isset($country["country"]["names"]["en"]) && $country["country"]["names"]["en"] == "Hashemite Kingdom of Jordan")	return "Jordan";
		return isset($country["country"]["names"]["en"]) ? strtolower($country["country"]["names"]["en"]) : "";			
	}
/**
 * Languages
 * @since 4.0
 **/	
  ...
  protected function avatar($user, $size = 150){
  	if($user->avatar) return "{$this->config["url"]}/content/avatar/{$user->avatar}";
  ...
  protected function report(){
		if(!$this->config["report"]) return $this->_404();
		...
		if(isset($_POST["token"])){
			...
			// Validate Token
			if(!Main::validate_csrf_token($_POST["token"])){
				return Main::redirect("report",array("danger",e("Something went wrong, please try again.")));
			}		
			if(empty($_POST["email"]) || !Main::email($_POST["email"])){
				return Main::redirect("report",array("danger",e("Please enter your email")."!"));			
			}
			if(empty($_POST["link"]) || !Main::is_url($_POST["link"])){
				return Main::redirect("report",array("danger",e("Please enter a valid url")."!"));	
			}
			...
			return Main::redirect("report",array("success",e("Thank you. We will review this link and take action.")));	
		}
		Main::set("title",e("Report Link"));
		Main::set("description",e("Please report a link that you consider risky or dangerous. We will review all cases and take measure to remove the link."));
		Main::set("url","{$this->config["url"]}/report");
  ...
  private function ProcessPayPalAPI(){
    ...
    $connection = new \PayPal\Rest\ApiContext(
  	...
  	$connection->setConfig(
		...
		$agreement->setName($plan->name)
		    ->setDescription('userid:'.$this->user->id)
    ...
    $payerInfo = new PayPal\Api\PayerInfo();
		$payerInfo->setEmail($this->user->email);
		$payerInfo->setFirstName($this->user->email);
  ...
  /**
	 * [webhook_paypal description]
	 * @author GemPixel <https://gempixel.com>
	 * @version 1.0
	 * @return  [type] [description]
	 */
	protected function webhook_paypal(){
