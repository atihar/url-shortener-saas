...
class Main{
  ...
  */  
    public static function ogp(){            
      ...
      if(!empty(self::$image)) $meta.="<meta property=\"og:image\" content=\"".self::image()."\" />\n\t"; 
      if(self::$config["twitter"]){
        $handler = explode("/", trim(strtolower(self::$config["twitter"]), "/"));
        $meta .= "<meta name=\"twitter:card\" content=\"summary_large_image\">\n\t"; 
        $meta .= "<meta name=\"twitter:site\" content=\"@".end($handler)."\">\n\t"; 
        $meta .= "<meta name=\"twitter:title\" content=\"".self::title()."\">\n\t"; 
        $meta .= "<meta name=\"twitter:description\" content=\"".self::description()."\">\n\t"; 
        $meta .= "<meta name=\"twitter:creator\" content=\"@".end($handler)."\">\n\t"; 
        if(!empty(self::$image)) $meta .= "<meta name=\"twitter:image:src\" content=\"".self::image()."\">\n\t"; 
      }
  ...
  */  
   ...
   * @return  [type]       [description]
   */
  public static function assets($file, $name = ""){
    return self::fstatic($file, $name);
  }       
  /**
  * Clean a string
 ...
 */
  ...
  */   
  ...
  /**
   * Slug Generator for custom alias
   * @author GemPixel <https://gempixel.com>
   * @version 5.8.3
   * @param   [type] $text [description]
   * @return  [type]       [description]
   */
  public static function customAlias($text){

      $str = preg_replace("/[\/_|+ -]+/", "-", $text);
      $str = str_replace("'","",$str);
      $str = str_replace('"','',$str);
      $str = rtrim(trim($str,'-'), '-');
    return $str;
  ...
  */  
    ...
    public static function timeago($time, $tense=TRUE){
     ...
     $now = time();
      ...
      return sprintf(e("%d {$periods[$j]} ago"), $difference);
    } 
...
*/  
  public static function check_captcha($array){
      if(self::$config["captcha"] == "1" || self::$config["captcha"] == "3" ){
        // Recaptcha
        if(empty($array["g-recaptcha-response"])) return e("Captcha is required. Please try again.");
        ...
        if($reCaptcha->verify() == false){
          return e("The captcha did not validate. Please try again.")."<script>recaptcha()</script>";
        }
      ...
      } elseif(self::$config["captcha"] == "4"){
        // hCaptcha
        if(empty($array["h-captcha-response"])) return e("Captcha is required. Please try again.");

        require_once(ROOT."/includes/library/hCaptcha.php");
        $hCaptcha = new hCaptcha(self::$config["captcha_private"]);
  ...
  */     
  public static function captcha(){
      ...
      }elseif(self::$config["captcha"] == "3"){          
        require_once(ROOT."/includes/library/Recaptcha.php");          
        return Recaptcha::renderV2(self::$config["captcha_public"]);

      }elseif(self::$config["captcha"] == "4"){
      // SolveMedia
      require_once(ROOT."/includes/library/hCaptcha.php");
      return hCaptcha::render(self::$config["captcha_public"]);      
    }
  }
...
/**
  ...
  */   
  ...
  /**
   * Crawler Detect Implementation
   * @author KBRmedia <https://gempixel.com>
   * @version 5.7.3
   * @return  [type]     [description]
   */
  public static function bot(){
    require_once(ROOT."/includes/library/Crawler/Fixtures/AbstractProvider.php");
    require_once(ROOT."/includes/library/Crawler/Fixtures/Crawlers.php");
    require_once(ROOT."/includes/library/Crawler/Fixtures/Exclusions.php");
    require_once(ROOT."/includes/library/Crawler/Fixtures/Headers.php");
    require_once(ROOT."/includes/library/Crawler/CrawlerDetect.php");
   ...
   * @since 4.1.1
   * @deprecated 5.7.3  Using Crawler Detect for better detection.
   */  
  public static function old_bot($ua=""){
 ...
 public static function ccode($code,$reverse=FALSE){
  ...
  public static function countries($code="united states"){
    $countries = ["AD"  => "Andorra","AE" => "United Arab Emirates","AF"  => "Afghanistan","AG" => "Antigua and Barbuda","AI" => "Anguilla","AL"  => "Albania","AM" => "Armenia","AN" => "Netherlands Antilles","AO"  => "Angola","AQ"  => "Antarctica","AR"  => "Argentina","AS" => "American Samoa","AT"  => "Austria","AU" => "Australia","AW" => "Aruba","AX" => "Åland","AZ" => "Azerbaijan","BA"  => "Bosnia and Herzegovina","BB"  => "Barbados","BD"  => "Bangladesh","BE"  => "Belgium","BF" => "Burkina Faso","BG"  => "Bulgaria","BH"  => "Bahrain","BI" => "Burundi","BJ" => "Benin","BL" => "Saint Barthélemy","BM"  => "Bermuda","BN" => "Brunei","BO"  => "Bolivia","BQ"   => "Bonaire, Sint Eustatius and Saba","BR"  => "Brazil","BS"  => "Bahamas","BT" => "Bhutan","BV"  => "Bouvet Island","BW" => "Botswana","BY"  => "Belarus","BZ" => "Belize","CA"  => "Canada","CC"  => "Cocos [Keeling] Islands","CD" => "DR Congo","CF"  => "Central African Republic","CG"  => "Congo Republic","CH"  => "Switzerland","CI" => "Ivory Coast","CK" => "Cook Islands","CL"  => "Chile","CM" => "Cameroon","CN"  => "China","CO" => "Colombia","CR"  => "Costa Rica","CS"  => "Serbia and Montenegro","CU" => "Cuba","CV"  => "Cabo Verde","CW"  => "Curaçao","CX" => "Christmas Island","CY"  => "Cyprus","CZ"  => "Czechia","DE" => "Germany","DJ" => "Djibouti","DK"  => "Denmark","DM" => "Dominica","DO"  => "Dominican Republic","DZ"  => "Algeria","EC" => "Ecuador","EE" => "Estonia","EG" => "Egypt","EH" => "Western Sahara","ER"  => "Eritrea","ES" => "Spain","ET" => "Ethiopia","FI"  => "Finland","FJ" => "Fiji","FK"  => "Falkland Islands","FM"  => "Micronesia","FO"  => "Faroe Islands","FR" => "France","GA"  => "Gabon","GB" => "United Kingdom","GD"  => "Grenada","GE" => "Georgia","GF" => "French Guiana","GG" => "Guernsey","GH"  => "Ghana","GI" => "Gibraltar","GL" => "Greenland","GM" => "Gambia","GN"  => "Guinea","GP"  => "Guadeloupe","GQ"  => "Equatorial Guinea","GR" => "Greece","GS"  => "South Georgia and South Sandwich Islands","GT"  => "Guatemala","GU" => "Guam","GW"  => "Guinea-Bissau","GY" => "Guyana","HK"  => "Hong Kong","HM" => "Heard Island and McDonald Islands","HN" => "Honduras","HR"  => "Croatia","HT" => "Haiti","HU" => "Hungary","ID" => "Indonesia","IE" => "Ireland","IL" => "Israel","IM"  => "Isle of Man","IN" => "India","IO" => "British Indian Ocean Territory","IQ"  => "Iraq","IR"  => "Iran","IS"  => "Iceland","IT" => "Italy","JE" => "Jersey","JM"  => "Jamaica","JO" => "Jordan","JP"  => "Japan","KE" => "Kenya","KG" => "Kyrgyzstan","KH"  => "Cambodia","KI"  => "Kiribati","KM"  => "Comoros","KN" => "St Kitts and Nevis","KP"  => "North Korea","KR" => "South Korea","KW" => "Kuwait","KY"  => "Cayman Islands","KZ"  => "Kazakhstan","LA"  => "Laos","LB"  => "Lebanon","LC" => "Saint Lucia","LI" => "Liechtenstein","LK" => "Sri Lanka","LR" => "Liberia","LS" => "Lesotho","LT" => "Lithuania","LU" => "Luxembourg","LV"  => "Latvia","LY"  => "Libya","MA" => "Morocco","MC" => "Monaco","MD"  => "Moldova","ME" => "Montenegro","MF"  => "Saint Martin","MG"  => "Madagascar","MH"  => "Marshall Islands","MK"  => "North Macedonia","ML" => "Mali","MM"  => "Myanmar","MN" => "Mongolia","MO"  => "Macao","MP" => "Northern Mariana Islands","MQ"  => "Martinique","MR"  => "Mauritania","MS"  => "Montserrat","MT"  => "Malta","MU" => "Mauritius","MV" => "Maldives","MW"  => "Malawi","MX"  => "Mexico","MY"  => "Malaysia","MZ"  => "Mozambique","NA"  => "Namibia","NC" => "New Caledonia","NE" => "Niger","NF" => "Norfolk Island","NG"  => "Nigeria","NI" => "Nicaragua","NL" => "Netherlands","NO" => "Norway","NP"  => "Nepal","NR" => "Nauru","NU" => "Niue","NZ"  => "New Zealand","OM" => "Oman","PA"  => "Panama","PE"  => "Peru","PF"  => "French Polynesia","PG"  => "Papua New Guinea","PH"  => "Philippines","PK" => "Pakistan","PL"  => "Poland","PM"  => "Saint Pierre and Miquelon","PN" => "Pitcairn Islands","PR"  => "Puerto Rico","PS" => "Palestine","PT" => "Portugal","PW"  => "Palau","PY" => "Paraguay","QA"  => "Qatar","RE" => "Réunion","RO" => "Romania","RS" => "Serbia","RU"  => "Russia","RW"  => "Rwanda","SA"  => "Saudi Arabia","SB"  => "Solomon Islands","SC" => "Seychelles","SD"  => "Sudan","SE" => "Sweden","SG"  => "Singapore","SH" => "Saint Helena","SI"  => "Slovenia","SJ"  => "Svalbard and Jan Mayen","SK"  => "Slovakia","SL"  => "Sierra Leone","SM"  => "San Marino","SN"  => "Senegal","SO" => "Somalia","SR" => "Suriname","SS"  => "South Sudan","ST" => "São Tomé and Príncipe","SV" => "El Salvador","SX" => "Sint Maarten","SY"  => "Syria","SZ" => "Eswatini","TC"  => "Turks and Caicos Islands","TD"  => "Chad","TF"  => "French Southern Territories","TG" => "Togo","TH"  => "Thailand","TJ"  => "Tajikistan","TK"  => "Tokelau","TL" => "Timor-Leste","TM" => "Turkmenistan","TN"  => "Tunisia","TO" => "Tonga","TR" => "Turkey","TT"  => "Trinidad and Tobago","TV" => "Tuvalu","TW"  => "Taiwan","TZ"  => "Tanzania","UA"  => "Ukraine","UG" => "Uganda","UM"  => "U.S. Minor Outlying Islands","US" => "United States","UY" => "Uruguay","UZ" => "Uzbekistan","VA"  => "Vatican City","VC"  => "St Vincent and Grenadines","VE" => "Venezuela","VG" => "British Virgin Islands","VI"  => "U.S. Virgin Islands","VN" => "Vietnam","VU" => "Vanuatu","WF" => "Wallis and Futuna","WS" => "Samoa","XK" => "Kosovo","YE"  => "Yemen","YT" => "Mayotte","ZA" => "South Africa","ZM"  => "Zambia","ZW"  => "Zimbabwe"];
    $form = "";
