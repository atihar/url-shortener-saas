...
class Short extends App{	
	...
	public function add($array=array(),$option=array()){
		// Kill the bots
		if(!isset($option["api"]) && !isset($option["user"])) {
    ...
    // If require registration
		...
		if(isset($this->user) && $this->user->plan->numurls && $this->user->plan->numurls > 0 && $this->count("user_urls") >= $this->user->plan->numurls) return array('error' => 1, 'msg' => e('You have reached your maximum links limit. Please upgrade to another plan.'));
		...
		if(empty($array["domain"]) && !$this->config["root_domain"]) {
			...
			if(!empty($mdomains[0])){
				$this->config["url"] = trim($mdomains[0]);
				$array["domain"] = trim($mdomains[0]);
			}else{
				$array["domain"] = $this->config["url"];
			}
		}
		...
		if(isset($array["custom"]) && !empty($array["custom"])){			
			...
			}elseif($this->db->get("url","custom=? AND (domain = ? OR domain = ?)","",array(Main::slug(Main::clean($array["custom"])),$array["domain"], ""))){
				return array('error' => 1, 'msg' => e('That alias is taken. Please choose another one.'));

			}elseif($this->db->get("url","alias=? AND (domain = ? OR domain = ?)","",array(Main::slug(Main::clean($array["custom"])),$array["domain"], ""))){
				return array('error' => 1, 'msg' => e('That alias is taken. Please choose another one.'));
		...
		if(isset($this->user) && $this->user->id && (!isset($array["password"]) || empty($array["password"])) &&  (!isset($array["custom"]) || empty($array["custom"]))){
			...
			if(empty($array["type"]) || !$array["type"]){
				...
				$data = $this->db->get("url",array("url"=>"?","userid"=>"?","location"=>"?","pass"=>"?", "devices" => "?", "pixels" => "?"), array("limit"=>"1"), array($url, $this->user->id, $countries, "", $devices, ""));
			}else{
				$data = $this->db->get("url",array("url"=>"?","userid"=>"?","location"=>"?","pass"=>"?", "devices" => "?", "pixels" => "?", "type" => "?"), array("limit"=>"1"), array($url, $this->user->id, $countries, "", $devices, "", $array["type"]));
			}
		...
		$alias = "";
		if(isset($array["custom"]) && !empty($array["custom"])){
			$custom = Main::customAlias(Main::clean($array["custom"],3,TRUE));
		}else{
			$alias = $this->alias();
		}
		...
		if($this->config["manualapproval"]) $data[":status"] = "0";
		...
		if($this->db->insert("url",$data)){
			...
			if(isset($this->user) && !empty($this->user->zapurl)){
				Main::curl($this->user->zapurl, [
						"json" => true,
						"post" => true,
						"body" => json_encode([
								"type" 			=> "url",
								"longurl" 	=> $data[":url"],
								"shorturl" 	=> $data[":domain"]."/".$data[":alias"].$data[":custom"],
	...
	private function redirect(){
		...
		if($_GET) {
			if(strpos($url->url, "?")){
				$url->url = $url->url."&".http_build_query($_GET);
			}else{
				$url->url = $url->url."?".http_build_query($_GET);
			}				
		}
		...
		if(!$url->status) return $this->custom404("NA");
		...
		if($this->blacklist_domain($url->url)) return $this->unsafe();
		...
		if($url->userid !=0 && $user = $this->db->get("user", ["id" => $url->userid], ["limit" => 1])){			
			...
			if($user->pro && time() > strtotime($user->expiration)) $this->db->update("user",array("pro" => 0, "trial" => 0),array("id"=>$user->id));
	...
	private function update($url){
		...
		if($this->config["tracking"]=="1"){
			...
			if(isset($user) && !empty($user->zapview)){
				Main::curl($user->zapview, [
						"json" => true,
						"post" => true,
						"body" => json_encode([
								"type" 			=> "view",
								"shorturl" 	=> $url->domain."/".$this->action,
								"country" 	=> $this->country(),
								"referer" 	=> $referer,
								// "ip" 				=> Main::ip(),
								"os" 				=> Main::os(),
	...
	private function overlay($url, $overlay){
        ...
        <div class='".($data->text ? 'notclickable' : 'clickable')."'>
          ...
          ".(isset($data->image) && $data->image? "<span class='custom-img'><img src='{$this->config["url"]}/content/{$data->image}' alt='{$data->message}'></span>" : "")."
	...
	private function splash($url){
		...
		if(!empty($this->config["timer"]) || $this->config["timer"] !=="0"){
			Main::add('<script type="text/javascript">var count = '.$this->config['timer'].';var countdown = setInterval(function(){$("a.redirect").attr("href","#pleasewait").html(count + " seconds");if (count < 1) {clearInterval(countdown);$("a.redirect").attr("href","'.$url->url.'").html("'.e('Continue').'");}count--;}, 1000);</script>',"custom",FALSE);     
		...
		$this->footer();
			...
			private function embed($data){
				...
				$sites=array(
					...
					"youtu" => "<iframe id='ytplayer' type='text/html'  width='640' height='400' allowtransparency='true' src='//www.youtube.com/embed/{$data->id}?autoplay=1&origin={$this->config["url"]}' frameborder='0'></iframe>",
					// Vimeo
	...
	private function stats(){			
		...
		Main::add("<script type='text/javascript' src='{$this->config["url"]}/analytic/".str_replace("=", "", base64_encode("{$url->id}:{$url->click}"))."?token={$this->config["public_token"]}' id='statslauncher'></script>", "custom");

		Main::cdn("daterangepicker");
		Main::add("<script>$(document).ready(function(){ 		
	 ...
	 * @since 5.9.2
	 */
	public function blacklist_keywords($url){
	 ...
	 * @since 5.9.2
	 */	
	public function url_framed($url){
	 ...
	 */
	public function reserved_alias($alias){
		// Check system alias
	 ...
	 * @since 4.0
	 **/
	public function premium_alias($alias){
 ...
 /**
	 ...
	 * @return  [type]     [description]
	 */
	protected function pixel_gtmpixel($id){
