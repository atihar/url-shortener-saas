...
class User extends App{	
	...
	protected function login(){
		if(!empty($this->id)){
			...
			if($this->id == "2FA") return $this->login_2FA();
	...
	protected function forgot(){
		...
		if(isset($this->id) && !empty($this->id)){
			...
			$unique = substr($new, 0, -32);
			$key = str_replace($unique, "", $new);

			// Check User
			if(!$user = $this->db->get("user", ["api" => "?"], ["limit" => 1], [$unique])) return Main::redirect(Main::href("user/forgot/{$this->id}","",FALSE),array("danger",e("Invalid token. Please try again.")));

			if($key == Main::encode($user->id.$this->config["security"].": Expires on".strtotime(date('Y-m-d')),"md5")){				
		...
		if(isset($_POST["token"])){
			...
			if($user=$this->db->get("user",array("email"=>"?","banned"=>"0"),array("limit"=>1),array($_POST["email"]))){
				// Generate key
				$forgot_url=Main::href("user/forgot/".str_replace("=","", base64_encode($user->api.Main::encode($user->id.$this->config["security"].": Expires on".strtotime(date('Y-m-d')),"md5"))));
	...
	protected function bundles(){
		if($this->permission("bundle") === FALSE) return Main::redirect("upgrade",array("warning",e("Please choose a premium package to unlock this feature.")));
  ...
  private function edit(){
  	...
  	if(isset($_POST["token"])){
			...
			if(isset($_POST["expiry"]) && !empty($_POST["expiry"]) && strtotime("now") > strtotime($_POST["expiry"])) return array('error' => 1, 'msg' => e('The expiry date must be later than today.'));
				...
				if($this->permission("alias") !== FALSE && isset($_POST["custom"]) && !empty($_POST["custom"]) && $_POST["custom"] != $url->custom){	
					...
					include_once(ROOT."/includes/Short.class.php");
					$shorten = new Short($this->db, $this->config);					

					if(strlen($_POST["custom"]) < 3){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('Custom alias must be at least 3 characters.')]);
					
					}elseif($short->blacklist_keywords($_POST["custom"])){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('Inappropriate aliases are not allowed.')]);

					}elseif($this->db->get("url","custom=? AND (domain = ? OR domain = ?)","",array(Main::slug(Main::clean($_POST["custom"])),$_POST["domain"], ""))){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('That alias is taken. Please choose another one.')]);

					}elseif($this->db->get("url","alias=? AND (domain = ? OR domain = ?)","",array(Main::slug(Main::clean($_POST["custom"])),$_POST["domain"], ""))){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('That alias is taken. Please choose another one.')]);

					}elseif($short->reserved_alias($_POST["custom"])){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('That alias is reserved. Please choose another one.')]);
					
					}elseif($short->premium_alias($_POST["custom"])){
						return Main::redirect(Main::href("user/edit/{$this->id}","",FALSE), ["danger", e('That is a premium alias and is reserved to only pro members.')]);
					}			

					$data[":custom"] = Main::clean($_POST["custom"]);
					$data[":alias"] = NULL;
				}
              ...
              <div class='form-group devices hide' style='display:none'>
               ...
               <div class='form-group parameters hide' style='display:none'>
                  <div class='col-sm-6'>
                    ...
                    <input type='text' class='form-control autofillparam' name='paramname[]' value=''>
                  </div>
                  <div class='col-sm-6'>
    ...
    $content="      
		...
		if($this->permission("parameters") !== FALSE){
	        ...
	        foreach ($parameters as $parameter => $value){
	          $content.="<div class='form-group'>
	                      <div class='col-sm-6'>
	                        <label>".e("Parameter Name")."</label>
                          <input type='text' class='form-control autofillparam' name='paramname[]' value='$parameter'>
	                      </div>
	                      <div class='col-sm-6'>
	                      	<label>".e("Parameter value")."</label>
	  ...
	  $content.="
	    ...
	    <a href='".Main::href("{$url->id}/stats")."' class='btn btn-success' target='_blank'>".e("Stats")."</a>
  ...
  private function splash(){
	  ...
	  $widgets='<div class="panel panel-default panel-body">';
			...
			if($count == "0"){
				...
				$widgets.="<p>".sprintf(e("A custom splash page is a transitional page where you can add a banner and an avatar along with a message to represent your brand or company. You can have up to a maximum of %s splash pages and you can choose one for each URL."), $count)."</p>";				
				if($this->pro() && $count > 0){
  	...
  	$this->footer();
		...
		private function splash_create(){
			...
			if(isset($_POST["token"])){
				...
				if(!in_array(Main::extension($_FILES["avatar"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/splash/create","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG.")));
				...
				if(!in_array(Main::extension($_FILES["banner"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/splash/create","",FALSE),array("danger",e("Banner must be either a PNG or a JPEG.")));
		...
		private function splash_edit(){
			...
			if(isset($_POST["token"])){
				...
				if(!empty($_FILES["avatar"]["name"])){
					...
					if(!in_array(Main::extension($_FILES["avatar"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/splash/{$this->id}","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG.")));
				...
				if(!empty($_FILES["banner"]["name"])){
					...
					if(!in_array(Main::extension($_FILES["banner"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/splash/{$this->id}","",FALSE),array("danger",e("Banner must be either a PNG or a JPEG.")));
  	...
  	if($this->permission("overlay") === FALSE) return Main::redirect("upgrade",array("warning",e("Please choose a premium package to unlock this feature.")));
			...
			if(isset($_POST["token"])){
				...
				if(isset($_FILES["logo"]["type"]) && !empty($_FILES["logo"]["type"])){
					...
					if(!in_array(Main::extension($_FILES["logo"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/overlay/message","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG.")));
  ...
  private function overlay_contact(){
	  ...
	  $widgets.='<div class="panel panel-default panel-body" id="'.__FUNCTION__.'">';
			...
			$widgets.="<p>".e("If you add a webhook url, we will send a notification to that url with the contact form data. You will be able to integrate it with your own app or a third-party app. Below is a sample data that will be sent in <code>JSON</code> format via a <code>POST</code> request.")."</p>";
  ...
  protected function tools(){
  	...
  	if($this->permission("api") === FALSE){
		...
		if($this->id == "regenerate"){
			...
			$api = Main::strrand(4).$this->user->id.Main::strrand(16);
		...
		// Developer API
		if($this->id == "api"){
			return $this->tools_api();
		}

		// Zapier
		if($this->id == "zapier"){			
			if(!isset($_POST["token"]) || !Main::validate_csrf_token($_POST["token"])) {
				return Main::redirect(Main::href("user/tools#zapier","",FALSE),array("danger",e("Something went wrong, please try again.")));
			}	

			$data[":zapurl"] = Main::clean($_POST["zapurl"], 3, true);
			$data[":zapview"] = Main::clean($_POST["zapview"], 3, true);

			if($data) $this->db->update("user", [], ["id" => $this->user->id], $data);

			return Main::redirect(Main::href("user/tools#zapier","",FALSE),array("success",e("Your Zapier URL has been updated.")));
		}		
		
		// Slack Integration
		if(!empty($this->config["slackclientid"]) && !empty($this->config["slacksecretid"])){
			include(ROOT."/includes/library/Slack.class.php");

			$slack = new kbrmedia\Slack($this->config["slackclientid"], $this->config["slacksecretid"], Main::href("user/tools/slack"));

			if($this->id == "slack"){		

				if($error = $slack->error()){
					return Main::redirect("user/tools#slack", ["danger", $error]);
				}
   ...
   */
	...
	protected function settings(){
		...
		if(isset($_POST["token"])){
			...
			if(isset($_FILES["avatar"]) && !empty($_FILES["avatar"]["tmp_name"])) {
				...
				if(!isset($ext[$_FILES["avatar"]["type"]])) return Main::redirect(Main::href("user/settings","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG.")));

				if(!in_array(Main::extension($_FILES["avatar"]["name"]), ['.png','.jpg'])) return Main::redirect(Main::href("user/settings","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG.")));

	      if($_FILES["avatar"]["size"] > 500*1024) return Main::redirect(Main::href("user/settings","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG (Max 500kb).")));   

	      list($width, $height) = getimagesize($_FILES["avatar"]["tmp_name"]);

	      if(($width < 100 && $height < 100) || ($width > 500 && $height > 500)) return Main::redirect(Main::href("user/settings","",FALSE),array("danger",e("Avatar must be either a PNG or a JPEG with a recommended dimension of 200x200.")));

				$data[":avatar"] = $this->user->id.Main::clean($_FILES["avatar"]["name"], 3, TRUE);
				move_uploaded_file($_FILES["avatar"]['tmp_name'], ROOT."/content/avatar/".$data[":avatar"]); 

				if(!empty($this->user->avatarpath) && file_exists(ROOT."/content/avatar/".$this->user->avatarpath)){
					unlink(ROOT."/content/avatar/".$this->user->avatarpath);
				}
			}
	...
	protected function export(){
  	...
  	if($this->permission("export") === FALSE){
	    ...
	    foreach ($result as $url) {
	     	echo ($url->domain ? $url->domain : $this->config["url"])."/{$url->alias}{$url->custom},{$url->url},{$url->date},{$url->click}\n";
	    }
	    return;
	}	
	/**
	 ...
	 * @since 5.9.2
	 */
	protected function export_data($url){
  	if($this->permission("export") === FALSE){
			return Main::redirect("upgrade",array("warning",e("Please choose a premium package to unlock this feature.")));
  	}		
		if(!Main::validate_nonce("export_url-{$this->id}")) return Main::redirect("user/edit/{$url->id}",array("danger",e("Security token expired, please try again.")));
  		header('Content-Type: text/csv');
	  	header('Content-Disposition: attachment;filename=URL_Shortener_'.$url->id.'_Stats.csv');
	 	 	$result = $this->db->get("stats",array("urluserid"=>$this->user->id,"urlid"=>$url->id),array("order"=>"id","all"=>1));
			echo "Short URL, Date, IP, Country, Browser, OS, Referrer\n";
	    foreach ($result as $stats) {
	     	echo ($url->domain ? $url->domain : $this->config["url"])."/{$url->alias}{$url->custom},{$stats->date},{$stats->ip},{$stats->country},{$stats->browser},{$stats->os},{$stats->referer}\n";
	    }
	    return;
	}			
	/**
	...
	protected function pixels(){
  	...
  	if($this->permission("pixels") === FALSE){
		...
		if(isset($_POST["token"])){
			...
			if($_POST["type"] == "gtm"){
				if((strlen($_POST["tag"]) < 5 || strpos($_POST["tag"], "GTM") === false)) return Main::redirect(Main::href("user/pixels","",FALSE),array("danger",e("GTM container ID is not correct. Please double check.")));		
				$key = "gtmpixel";	
			}						
  	...
  	if(is_numeric($this->id)) return $this->domain_delete();
		...
		if(isset($_POST["token"])){
			...
			$data = [
				...
				":redirect404" => Main::clean($_POST["default404"], 3, TRUE),
				":status" => "1"
			];
	...
	protected function teams(){
  	...
  	if($count  === FALSE){
		...
		$toText = [
			...
			"splash.create" => e("Create Splash"),
			"splash.edit" => e("Edit Splash"),
			"splash.delete" => e("Delete Splash"),				    				
			"overlay.create" => e("Create Overlay"),
			"overlay.edit" => e("Edit Overlay"),
			"overlay.delete" => e("Delete Overlay"),				    
			"pixels.create" => e("Create Pixels"),
			"pixels.edit" => e("Edit Pixels"),
			"pixels.delete" => e("Delete Pixels"),				    				
			"domain.create" => e("Add Custom Domain"),
			"domain.delete" => e("Delete Custom Domain"),				    
			"bundle.create" => e("Create Bundles"),
			"bundle.edit" => e("Edit Bundles"),
			"bundle.delete" => e("Delete Bundles"),				    				
			"api.create" => e("Developer API"),		    				
   ...
   return Main::redirect(Main::href("user/teams","",FALSE),array("danger",e("Something went wrong, please try again.")));
	...
	protected function builder(){
		...
		if(isset($_POST["token"])){
			if(!Main::validate_csrf_token($_POST["token"])){
				return Main::redirect(Main::href("user/builder","",FALSE),array("danger",e("Invalid token. Please try again.")));
			}	

			$data = [
				"name" => Main::clean($_POST["name"], 3, TRUE),
				"scheme" => Main::clean($_POST["scheme"], 3, TRUE),
				"all" => in_array($_POST["all"], [0, 1]) ? $_POST["all"] : 0,
				"bundle" => in_array($_POST["bundle"], [0, 1]) ? $_POST["bundle"] : 0,
			];
			
			include("includes/Short.class.php");				
			$shorten = new Short($this->db,$this->config);

			foreach ($_POST["links"] as $i => $link) {
				if(empty($link) && empty($_POST["text"][$i])) continue;
				$link = Main::clean($link, 3, TRUE);

				if($url = $this->db->get("url", ["userid" => "?", "url" => "?"], ["limit" => 1], [$this->user->id, $link])){

					$short = (empty($url->domain) ? $this->config["url"] : $url->domain)."/".$url->alias.$url->custom;

				} else{
					
					$array["url"] = Main::clean($link, 3, TRUE);

					$result = $shorten->add($array, ["noreturn" => TRUE, "api" => TRUE, "user" => $this->user]);
					if($result["error"]) continue;
					$short = $result["short"];
				}

				$data["links"][] = [
					"text" => Main::clean($_POST["text"][$i], 3, TRUE),
					"link" =>  $link,
					"short" => $short
				];
			}
		...
		$profile = json_decode($this->user->profiledata, TRUE);
		
		if(!isset($profile["scheme"])) $profile["scheme"] = "default";
		if(!isset($profile["name"])) $profile["name"] = "";
		if(!isset($profile["bundle"])) $profile["bundle"] = 0;
		if(!isset($profile["links"])) $profile["links"] = [];
		if(!isset($profile["all"])) $profile["all"] = 0;

		if($this->sandbox == TRUE) {
			// Developement Stylesheets
			Main::add("<link rel='stylesheet/less' type='text/css' href='{$this->config["url"]}/themes/{$this->config["theme"]}/assets/css/widgets.less'>","custom",false);
			Main::cdn("less");
		} else{
			Main::add("<link rel='stylesheet' type='text/css' href='{$this->config["url"]}/themes/{$this->config["theme"]}/assets/css/widgets.css'>","custom",false);
		}

		Main::set("title", e("Profile Builder"));


		$this->isUser=TRUE;
  	$this->header();
		include($this->t("builder"));
  	$this->footer();	
	}
	/**
	 * [tools_api description]
	 * @author GemPixel <https://gempixel.com>
	 * @version 1.0
	 * @return  [type] [description]
	 */
	protected function tools_api(){

		Main::set("title", e("Developer API"));
		Main::cdn("hljs");
		Main::add("<script>hljs.initHighlightingOnLoad();</script>", "custom", "footer");
		$this->isUser=TRUE;
  	$this->header();
		include($this->t("api"));
  	$this->footer();		
